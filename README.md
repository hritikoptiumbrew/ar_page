...
- `menu/index.html` (Accessible via `/menu`): A menu page listing all available models.
- `viewer/index.html` (Accessible via `/viewer`): The dynamic AR viewer page.
...
3.  Access the model by adding the `model` parameter to the URL:
    - `https://menuxr.in/viewer?model=myModel`

## Local Development
1.  Serve the project using a local web server:
    - `python3 -m http.server 8081`
2.  Open in browser:
    - `http://localhost:8081/menu` (Menu)
    - `http://localhost:8081/viewer?model=vegBurger` (Direct Viewer)
